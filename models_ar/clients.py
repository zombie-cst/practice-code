...
class Clients:
    ...
    def get_clients_by_id(clients_id):
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("""SELECT id, firstName, lastName, patronymic, addres,
                    phoneNumber FROM Clients WHERE id = ? """, (clients_id,))
        row = cursor.fetchone()
        conn.close()
        if row:
            return Clients(id=row[0], firstName=row[1], lastName=row[2], patronymic=row[3],
                           addres=row[4], phoneNumber=row[5])
        return None