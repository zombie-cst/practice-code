...
class Booking:
    def __init__(self, id=None, date_issue=None, return_date=None,
                 addres=None, automobiles_id=None, clients_id=None, employees_id=None):
        self.id = id
    ...
    def save(self):
        ...
        if self.id is None:
            cur.execute('''INSERT INTO Booking(date_issue, return_date,
                        addres, automobiles_id, clients_id, employees_id)
                        VALUES(?, ?, ?, ?, ?, ?)''',
                        (self.date_issue, self.return_date, self.addres,
                         self.automobiles_id, self.clients_id, self.employees_id))
...
def get_all_booking():
    ...
    cur.execute('''SELECT id, date_issue, return_date, addres,
                automobiles_id, clients_id, employees_id FROM Booking''')
    rows = cur.fetchall()
    conn.close()
    return [Booking(id=row[0], date_issue=row[1], return_date=row[2], addres=row[3],
